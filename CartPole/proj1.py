...
# ====================
def do_play(EPSILON):
    logf = open("rewards_playing.txt", "w")
    env = gym.make("CartPole-v1", render_mode="human")

    trained_params = init_params_from_file("weights.pkl")
...
def do_train(EPSILON):
    ...
    for episode in range(NUM_EPISODES):
        ...
        while not episode_done:
            ...
            action = policy(key, qnet_params, state, EPSILON, env.action_space)
    ...
    with open("weights.pkl", "wb") as wf:
        jnp.savez(
            wf,
            w1=qnet_params["w1"],
            w2=qnet_params["w2"],
